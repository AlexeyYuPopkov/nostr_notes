...
import 'package:common/services/event_store/database/tables/nostr_events.dart';
import 'package:common/services/event_store/database/tables/nostr_tags.dart';
...
class NostrEventDao extends DatabaseAccessor<AppDatabase>
  ...
  Future<void> upsertEvents(Iterable<NostrEvent> events) async {
    ...
    await transaction(() async {
      ...
      await batch((b) {
        ...
        // 4) Record which relay each event came from.
        _insertRelayInfo(events, b, nowMs);
      });
  ...
  void _insertRelayInfo(Iterable<NostrEvent> events, Batch batch, int nowMs) {
    for (final e in events) {
      if (e is NostrEventWithRelay && e.relay.isNotEmpty) {
        batch.insert(
          nostrEventRelays,
  ...
  Future<Set<String>> eventRelays(NostrEvent event) {
    final rows = (select(
      nostrEventRelays,
    )..where((r) => r.eventId.equals(event.id))).get();

    return rows.then((list) {
      return list.map((r) => r.relayUrl).where((e) => e.isNotEmpty).toSet();
    });
  }
}
